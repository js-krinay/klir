...
class TestPathFromFileTag:
    ...
    def test_docker_path_translated(self) -> None:
        with patch.dict("os.environ", {"KLIR_HOME": "/home/user/.klir"}):
            with patch("klir.files.tags.is_windows", return_value=False):
                path = path_from_file_tag("/klir/workspace/output_to_user/img.png")
        assert path == Path("/home/user/.klir/workspace/output_to_user/img.png")

    def test_docker_path_root(self) -> None:
        with patch.dict("os.environ", {"KLIR_HOME": "/home/user/.klir"}):
            with patch("klir.files.tags.is_windows", return_value=False):
                path = path_from_file_tag("/klir/sessions.json")
        assert path == Path("/home/user/.klir/sessions.json")
