...
async def test_discover_models_parses_response() -> None:
    proc = _mock_process()
    with (
        patch("klir.cli.codex_discovery.which", return_value="/usr/bin/codex"),
...
async def test_discover_models_codex_not_installed() -> None:
    with patch("klir.cli.codex_discovery.which", return_value=None):
        models = await discover_codex_models()
    assert models == []
